...
#        print "miss", self, name
    ...
    def descr_get_annotations(self, space):
        # CPython checks this before the dict lookup: for `type` itself the
        # dict contains the __annotations__ getset descriptor, which must
        # not be returned raw (test_funcattrs/test_functools access
        # type.__annotations__ and expect AttributeError)
        if not self.is_heaptype():
            raise oefmt_attribute_error(
