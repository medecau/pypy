...
class PythonCodeGenerator(assemble.PythonCodeMaker):
    ...
    def _compile_inlined_comprehension(self, node, names):
        ...
        #
        # The handler is an exception-table scope, as CPython's is: the
        # _SETUP_FINALLY range covers the body, the handler is entered with
        # just [exc], restores the saved slots and RERAISE 0s.  (CPython's
        # compiler_comprehension does exactly this; no PUSH_EXC_INFO,
        # because nothing here observes sys.exc_info.)
        ...
        self.emit_jump(_SETUP_FINALLY, handler)
        self.use_next_block()
        ...
        # entered with [exc]; the restore pairs and the iterator pop are all
        # TOS-neutral, then RERAISE 0 pops exc and propagates
        self.no_position_info()
        self._emit_inlined_comp_restore(counter, names, iter_name)
        self.emit_op_arg(ops.RERAISE, 0)
        self.use_next_block(end)
