...
class PythonParser(Parser):
    ...
    def import_stmt(self): # type Optional[ast . Import]
        ...
        if self.call_invalid_rules:
            invalid_import = self.invalid_import()
            if invalid_import:
                assert 0, 'unreachable'
            self._index = mark
            ...
            if a:
                ...
                if literal_1:
                    ...
                    if c:
                        tok = self.get_last_non_whitespace_token()
                        end_lineno, end_col_offset = tok.end_lineno, tok.end_column
                        return ast . ClassDef ( self . extract_id ( a ) , bases = b . args if b else None , keywords = b . keywords if b else None , body = c , decorator_list = None , type_params = t , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset , )
        self._index = mark
        return None
                    ...
                    if literal_2:
                        ...
                        if literal_3:
                            ...
                            if b:
                                tok = self.get_last_non_whitespace_token()
                                end_lineno, end_col_offset = tok.end_lineno, tok.end_column
                                return ast . FunctionDef ( name = self . extract_id ( n ) , args = params or self . make_arguments ( None , None , None , [] , None ) , returns = a , body = b , decorator_list = None , type_comment = tc , type_params = t , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset , )
        self._index = mark
    ...
    def with_item(self): # type Optional[ast . withitem]
        ...
        if e:
            ...
            if literal:
                t = self.star_target()
                if t:
                    if self.positive_lookahead(PythonParser._tmp_61, ):
    ...
    def literal_pattern(self): # type Optional[Any]
        ...
        if value:
            if self.negative_lookahead(PythonParser._tmp_69, ):
                tok = self.get_last_non_whitespace_token()
                end_lineno, end_col_offset = tok.end_lineno, tok.end_column
    ...
    def type_alias(self): # type Optional[ast . TypeAlias]
        # type_alias: "type" NAME type_params? '=' expression
        mark = self._index
        if self._verbose: log_start(self, 'type_alias')
        tok = self.peek()
        start_lineno, start_col_offset = tok.lineno, tok.column
            ...
            if n:
                ...
                if literal_1:
                    ...
                    if b:
                        tok = self.get_last_non_whitespace_token()
                        end_lineno, end_col_offset = tok.end_lineno, tok.end_column
                        return self . check_version ( ( 3 , 12 ) , "Type statement is" , ast . TypeAlias ( self . set_expr_context ( n , Store ) , type_params = t , value = b , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset , ) )
        self._index = mark
        return None

    def type_params(self): # type Optional[list [ast . TypeVar | ast . TypeVarTuple | ast . ParamSpec]]
    ...
    def type_param_seq(self): # type Optional[list [ast . TypeVar | ast . TypeVarTuple | ast . ParamSpec]]
        # type_param_seq: ','.type_param+ ','?
        mark = self._index
        if self._verbose: log_start(self, 'type_param_seq')
        a = self._gather_82()
        if a:
            opt = self.expect_type(15)
            return a
    ...
    @memoize
    def type_param(self): # type Optional[ast . TypeVar | ast . TypeVarTuple | ast . ParamSpec]
        # type_param: NAME type_param_bound? | '*' NAME ':' expression | '*' NAME | '**' NAME ':' expression | '**' NAME
        mark = self._index
        if self._verbose: log_start(self, 'type_param')
        tok = self.peek()
        start_lineno, start_col_offset = tok.lineno, tok.column
        a = self.name()
        if a:
            ...
            tok = self.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end_lineno, tok.end_column
            return ast . TypeVar ( a . id , bound = b , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset )
        self._index = mark
        literal = self.expect_type(19)
        if literal:
            a = self.name()
        ...
        if literal:
            ...
            if a:
                tok = self.get_last_non_whitespace_token()
                end_lineno, end_col_offset = tok.end_lineno, tok.end_column
                return ast . TypeVarTuple ( a . id , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset )
        self._index = mark
        literal = self.expect_type(39)
        if literal:
            a = self.name()
        ...
        if literal:
            ...
            if a:
                tok = self.get_last_non_whitespace_token()
                end_lineno, end_col_offset = tok.end_lineno, tok.end_column
                return ast . ParamSpec ( a . id , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset )
        self._index = mark
        return None

    def type_param_bound(self): # type Optional[ast . Expression]
        # type_param_bound: ':' expression
        mark = self._index
        if self._verbose: log_start(self, 'type_param_bound')
        literal = self.expect_type(14)
        if literal:
            e = self.expression()
            if e:
    ...
    def atom(self): # type Optional[Any]
        ...
        self._index = mark
        if self.positive_lookahead(PythonParser._tmp_94, ):
            strings = self.strings()
            if strings:
                return strings
        self._index = mark
    ...
    def starred_expression(self): # type Optional[Any]
        ...
        start_lineno, start_col_offset = tok.lineno, tok.column
        if self.call_invalid_rules:
            invalid_starred_expression = self.invalid_starred_expression()
            if invalid_starred_expression:
                assert 0, 'unreachable'
            self._index = mark
        literal = self.expect_type(19)
        if literal:
    ...
    def func_type_comment(self): # type Optional[Any]
        ...
        if _newline:
            t = self.type_comment()
            if t:
                if self.positive_lookahead(PythonParser._tmp_150, ):
                    return t
        self._index = mark
        if self.call_invalid_rules:
    ...
    def invalid_arguments(self): # type Optional[NoReturn]
        # invalid_arguments: ','.(starred_expression | (assignment_expression | expression !':=') !'=')+ ',' kwargs ',' ','.(starred_expression !'=')+ | kwargs ',' ','.(starred_expression !'=')+ | expression for_if_clauses ',' [args | expression for_if_clauses] | NAME '=' expression for_if_clauses | [(args ',')] NAME '=' &(',' | ')') | args for_if_clauses | args ',' expression for_if_clauses | args ',' args
        mark = self._index
        ...
        if a:
            b = self.expect_type(25)
            if b:
                if self.positive_lookahead(PythonParser._tmp_159, ):
    ...
    def invalid_kwarg(self): # type Optional[NoReturn]
        # invalid_kwarg: NAME '=' expression for_if_clauses | !(NAME '=') expression '=' | '**' expression '=' expression
        mark = self._index
    ...
    def invalid_expression(self): # type Optional[NoReturn]
        ...
        if a:
            ...
            if literal:
                b = self.disjunction()
                if b:
                    if self.negative_lookahead(PythonParser._tmp_162, ):
    ...
    def invalid_named_expression(self): # type Optional[NoReturn]
        ...
        if a:
            ...
            if literal:
                b = self.bitwise_or()
                if b:
                    if self.negative_lookahead(PythonParser._tmp_163, ):
        ...
        self._index = mark
        if self.negative_lookahead(PythonParser._tmp_164, ):
            a = self.bitwise_or()
            if a:
                b = self.expect_type(25)
                if b:
                    bitwise_or = self.bitwise_or()
                    if bitwise_or:
                        if self.negative_lookahead(PythonParser._tmp_165, ):
    ...
    def invalid_default(self): # type Optional[Any]
        ...
        if a:
            if self.positive_lookahead(PythonParser._tmp_183, ):
    ...
    def invalid_lambda_parameters(self): # type Optional[NoReturn]
        ...
        self._index = mark
        opt = self._tmp_196()
        _loop0_197 = self._loop0_197()
        literal = self.expect_type(19)
        if literal:
            _tmp_198 = self._tmp_198()
            if _tmp_198:
                _loop0_199 = self._loop0_199()
                a = self.expect_type(20)
                if a:
                    return self . raise_syntax_error_known_location ( "/ must be ahead of *" , a )
        self._index = mark
        _loop1_200 = self._loop1_200()
        if _loop1_200:
            literal = self.expect_type(20)
            if literal:
                a = self.expect_type(19)
    ...
    def invalid_with_item(self): # type Optional[NoReturn]
        ...
        if expression:
            ...
            if literal:
                a = self.expression()
                if a:
                    if self.positive_lookahead(PythonParser._tmp_207, ):
                        return self . raise_syntax_error_if_invalid_target ( a , "assign" )
        self._index = mark
        return None
    ...
    def invalid_import(self): # type Optional[Any]
        # invalid_import: 'import' ','.dotted_name+ 'from' dotted_name
        mark = self._index
        if self._verbose: log_start(self, 'invalid_import')
        a = self.expect_type(512)
        if a:
    ...
    def invalid_try_stmt(self): # type Optional[Any]
        ...
        if literal:
            ...
            if literal_1:
                block = self.block()
                if block:
                    if self.negative_lookahead(PythonParser._tmp_218, ):
        ...
        if literal:
            literal_1 = self.expect_type(14)
            if literal_1:
                _loop0_219 = self._loop0_219()
                _loop1_220 = self._loop1_220()
                if _loop1_220:
        ...
        if literal:
            literal_1 = self.expect_type(14)
            if literal_1:
                _loop0_222 = self._loop0_222()
                _loop1_223 = self._loop1_223()
                if _loop1_223:
                    a = self.expect_type(522)
                    if a:
                        opt = self._tmp_224()
    ...
    def invalid_double_starred_kvpairs(self): # type Optional[None]
        ...
        if expression:
            ...
            if a:
                if self.positive_lookahead(PythonParser._tmp_236, ):
    ...
    def invalid_starred_expression(self): # type Optional[NoReturn]
        # invalid_starred_expression: '*' expression '=' expression
        mark = self._index
        if self._verbose: log_start(self, 'invalid_starred_expression')
        a = self.expect_type(19)
        if a:
            expression = self.expression()
            if expression:
    ...
    def invalid_replacement_field(self): # type Optional[Any]
        ...
        literal = self.expect_type(29)
        if literal:
            if self.negative_lookahead(PythonParser._tmp_237, ):
        ...
        if literal:
            ...
            if _tmp_240:
                literal_1 = self.expect_type(25)
                if literal_1:
                    if self.negative_lookahead(PythonParser._tmp_241, ):
        ...
        if literal:
            ...
            if _tmp_243:
                opt = self.expect_type(25)
                opt_1 = self._tmp_244()
                if self.negative_lookahead(PythonParser._tmp_245, ):
    ...
    def invalid_conversion_character(self): # type Optional[Any]
        ...
        if literal:
            if self.positive_lookahead(PythonParser._tmp_251, ):
    ...
    def _loop0_54(self): # type Optional[Any]
        # _loop0_54: ',' with_item
        mark = self._index
        if self._verbose: log_start(self, '_loop0_54')
        children = []
        while True:
            literal = self.expect_type(15)
            if not literal:
    ...
    def _gather_53(self): # type Optional[Any]
        # _gather_53: with_item _loop0_54
        mark = self._index
        if self._verbose: log_start(self, '_gather_53')
        elem = self.with_item()
        if elem is not None:
            seq = self._loop0_54()
            if seq is not None:
    ...
    def _loop0_56(self): # type Optional[Any]
        # _loop0_56: ',' with_item
        mark = self._index
        if self._verbose: log_start(self, '_loop0_56')
        children = []
        while True:
            literal = self.expect_type(15)
            if not literal:
    ...
    def _gather_55(self): # type Optional[Any]
        # _gather_55: with_item _loop0_56
        mark = self._index
        if self._verbose: log_start(self, '_gather_55')
        elem = self.with_item()
        if elem is not None:
            seq = self._loop0_56()
            if seq is not None:
    ...
    def _loop0_58(self): # type Optional[Any]
        # _loop0_58: ',' with_item
        mark = self._index
        if self._verbose: log_start(self, '_loop0_58')
        children = []
        while True:
            literal = self.expect_type(15)
            if not literal:
    ...
    def _gather_57(self): # type Optional[Any]
        # _gather_57: with_item _loop0_58
        mark = self._index
        if self._verbose: log_start(self, '_gather_57')
        elem = self.with_item()
        if elem is not None:
            seq = self._loop0_58()
            if seq is not None:
    ...
    def _gather_59(self): # type Optional[Any]
        # _gather_59: with_item _loop0_60
        mark = self._index
        if self._verbose: log_start(self, '_gather_59')
        elem = self.with_item()
        if elem is not None:
            seq = self._loop0_60()
            if seq is not None:
    ...
    def _tmp_61(self): # type Optional[Any]
        # _tmp_61: ',' | ')' | ':'
        mark = self._index
        if self._verbose: log_start(self, '_tmp_61')
        literal = self.expect_type(15)
        if literal:
            return literal
        self._index = mark
    ...
    def _loop1_62(self): # type Optional[Any]
        # _loop1_62: except_block
        mark = self._index
        if self._verbose: log_start(self, '_loop1_62')
        children = []
        while True:
    ...
    def _tmp_64(self): # type Optional[Any]
        # _tmp_64: 'as' NAME
        mark = self._index
        if self._verbose: log_start(self, '_tmp_64')
        literal = self.expect_type(521)
        if literal:
            z = self.name()
            if z:
    ...
    def _loop1_66(self): # type Optional[Any]
        # _loop1_66: case_block
        mark = self._index
        if self._verbose: log_start(self, '_loop1_66')
        children = []
        while True:
    ...
    def _loop0_76(self): # type Optional[Any]
        ...
        mark = self._index
        if self._verbose: log_start(self, '_loop0_76')
        children = []
        while True:
            literal = self.expect_type(15)
            if not literal:
    ...
    def _gather_75(self): # type Optional[Any]
        ...
        mark = self._index
        if self._verbose: log_start(self, '_gather_75')
        elem = self.key_value_pattern()
        if elem is not None:
            seq = self._loop0_76()
            if seq is not None:
    ...
    def _gather_78(self): # type Optional[Any]
        # _gather_78: pattern _loop0_79
        mark = self._index
        if self._verbose: log_start(self, '_gather_78')
        elem = self.pattern()
        if elem is not None:
            seq = self._loop0_79()
            if seq is not None:
    ...
    def _gather_80(self): # type Optional[Any]
        # _gather_80: keyword_pattern _loop0_81
        mark = self._index
        if self._verbose: log_start(self, '_gather_80')
        elem = self.keyword_pattern()
        if elem is not None:
            seq = self._loop0_81()
            if seq is not None:
    ...
    def _gather_82(self): # type Optional[Any]
        # _gather_82: type_param _loop0_83
        mark = self._index
    ...
    def _loop0_86(self): # type Optional[Any]
        # _loop0_86: ',' star_named_expression
        mark = self._index
        if self._verbose: log_start(self, '_loop0_86')
        children = []
        while True:
            literal = self.expect_type(15)
            if not literal:
    ...
    def _gather_85(self): # type Optional[Any]
        # _gather_85: star_named_expression _loop0_86
        mark = self._index
        if self._verbose: log_start(self, '_gather_85')
        elem = self.star_named_expression()
        if elem is not None:
            seq = self._loop0_86()
            if seq is not None:
    ...
    def _loop0_92(self): # type Optional[Any]
        # _loop0_92: ',' (slice | starred_expression)
        mark = self._index
        if self._verbose: log_start(self, '_loop0_92')
        children = []
        while True:
            literal = self.expect_type(15)
            if not literal:
    ...
    def _gather_91(self): # type Optional[Any]
        # _gather_91: (slice | starred_expression) _loop0_92
        mark = self._index
        if self._verbose: log_start(self, '_gather_91')
        elem = self._tmp_259()
        if elem is not None:
            seq = self._loop0_92()
            if seq is not None:
    ...
    def _loop0_101(self): # type Optional[Any]
        ...
        mark = self._index
        if self._verbose: log_start(self, '_loop0_101')
        children = []
        while True:
            lambda_param_with_default = self.lambda_param_with_default()
            if not lambda_param_with_default:
    ...
    def _loop1_102(self): # type Optional[Any]
        ...
        mark = self._index
        if self._verbose: log_start(self, '_loop1_102')
        children = []
        while True:
            lambda_param_no_default = self.lambda_param_no_default()
            if not lambda_param_no_default:
    ...
    def _loop1_106(self): # type Optional[Any]
        ...
        mark = self._index
        if self._verbose: log_start(self, '_loop1_106')
        children = []
        while True:
            lambda_param_no_default = self.lambda_param_no_default()
            if not lambda_param_no_default:
    ...
    def _loop0_111(self): # type Optional[Any]
        # _loop0_111: lambda_param_maybe_default
        mark = self._index
        if self._verbose: log_start(self, '_loop0_111')
        children = []
        while True:
    ...
    def _tmp_113(self): # type Optional[Any]
        # _tmp_113: yield_expr | star_expressions
        mark = self._index
        if self._verbose: log_start(self, '_tmp_113')
        yield_expr = self.yield_expr()
        if yield_expr:
            return yield_expr
        self._index = mark
    ...
    def _gather_148(self): # type Optional[Any]
        # _gather_148: expression _loop0_149
        mark = self._index
        if self._verbose: log_start(self, '_gather_148')
        elem = self.expression()
        if elem is not None:
            seq = self._loop0_149()
            if seq is not None:
    ...
    def _loop0_152(self): # type Optional[Any]
        # _loop0_152: ',' (starred_expression | (assignment_expression | expression !':=') !'=')
        mark = self._index
    ...
    def _gather_151(self): # type Optional[Any]
        # _gather_151: (starred_expression | (assignment_expression | expression !':=') !'=') _loop0_152
        mark = self._index
    ...
    def _gather_155(self): # type Optional[Any]
        # _gather_155: (starred_expression !'=') _loop0_156
        mark = self._index
        if self._verbose: log_start(self, '_gather_155')
        elem = self._tmp_268()
        if elem is not None:
            seq = self._loop0_156()
            if seq is not None:
    ...
    def _tmp_157(self): # type Optional[Any]
        # _tmp_157: args | expression for_if_clauses
        mark = self._index
    ...
    def _tmp_164(self): # type Optional[Any]
        # _tmp_164: list | tuple | genexp | 'True' | 'None' | 'False'
        mark = self._index
    ...
    def _loop0_166(self): # type Optional[Any]
        # _loop0_166: star_named_expressions
        mark = self._index
    ...
    def _tmp_169(self): # type Optional[Any]
        # _tmp_169: yield_expr | star_expressions
        mark = self._index
    ...
    def _loop1_177(self): # type Optional[Any]
        # _loop1_177: param_no_default
        mark = self._index
        if self._verbose: log_start(self, '_loop1_177')
        children = []
        while True:
            param_no_default = self.param_no_default()
            if not param_no_default:
    ...
    def _loop1_182(self): # type Optional[Any]
        # _loop1_182: param_maybe_default
        mark = self._index
        if self._verbose: log_start(self, '_loop1_182')
        children = []
        while True:
            param_maybe_default = self.param_maybe_default()
            if not param_maybe_default:
    ...
    def _loop1_189(self): # type Optional[Any]
        # _loop1_189: param_with_default
        mark = self._index
        if self._verbose: log_start(self, '_loop1_189')
        children = []
        while True:
            param_with_default = self.param_with_default()
            if not param_with_default:
    ...
    def _gather_194(self): # type Optional[Any]
        # _gather_194: lambda_param _loop0_195
        mark = self._index
        if self._verbose: log_start(self, '_gather_194')
        elem = self.lambda_param()
        if elem is not None:
            seq = self._loop0_195()
            if seq is not None:
    ...
    def _tmp_196(self): # type Optional[Any]
        # _tmp_196: lambda_slash_no_default | lambda_slash_with_default
        mark = self._index
    ...
    def _loop0_197(self): # type Optional[Any]
        # _loop0_197: lambda_param_maybe_default
        mark = self._index
        if self._verbose: log_start(self, '_loop0_197')
        children = []
        while True:
            lambda_param_maybe_default = self.lambda_param_maybe_default()
            if not lambda_param_maybe_default:
    ...
    def _loop0_199(self): # type Optional[Any]
        # _loop0_199: lambda_param_maybe_default
        mark = self._index
        if self._verbose: log_start(self, '_loop0_199')
        children = []
        while True:
            lambda_param_maybe_default = self.lambda_param_maybe_default()
            if not lambda_param_maybe_default:
    ...
    def _loop1_200(self): # type Optional[Any]
        # _loop1_200: lambda_param_maybe_default
        mark = self._index
        if self._verbose: log_start(self, '_loop1_200')
        children = []
        while True:
            lambda_param_maybe_default = self.lambda_param_maybe_default()
            if not lambda_param_maybe_default:
    ...
    def _loop1_201(self): # type Optional[Any]
        # _loop1_201: lambda_param_with_default
        mark = self._index
    ...
    def _loop0_204(self): # type Optional[Any]
        # _loop0_204: lambda_param_maybe_default
        mark = self._index
        if self._verbose: log_start(self, '_loop0_204')
        children = []
        while True:
            lambda_param_maybe_default = self.lambda_param_maybe_default()
            if not lambda_param_maybe_default:
    ...
    def _tmp_205(self): # type Optional[Any]
        # _tmp_205: lambda_param_no_default | ','
        mark = self._index
    ...
    def _gather_208(self): # type Optional[Any]
        # _gather_208: dotted_name _loop0_209
        mark = self._index
    ...
    def _gather_212(self): # type Optional[Any]
        # _gather_212: (expressions ['as' star_target]) _loop0_213
        mark = self._index
        if self._verbose: log_start(self, '_gather_212')
        elem = self._tmp_274()
        if elem is not None:
            seq = self._loop0_213()
            if seq is not None:
    ...
    def _loop0_215(self): # type Optional[Any]
        # _loop0_215: ',' (expression ['as' star_target])
        mark = self._index
        if self._verbose: log_start(self, '_loop0_215')
        children = []
        while True:
            literal = self.expect_type(15)
            if not literal:
    ...
    def _gather_214(self): # type Optional[Any]
        # _gather_214: (expression ['as' star_target]) _loop0_215
        mark = self._index
        if self._verbose: log_start(self, '_gather_214')
        elem = self._tmp_275()
        if elem is not None:
            seq = self._loop0_215()
            if seq is not None:
    ...
    def _loop0_217(self): # type Optional[Any]
        # _loop0_217: ',' (expressions ['as' star_target])
        mark = self._index
        if self._verbose: log_start(self, '_loop0_217')
        children = []
        while True:
            literal = self.expect_type(15)
            if not literal:
    ...
    def _gather_216(self): # type Optional[Any]
        # _gather_216: (expressions ['as' star_target]) _loop0_217
        mark = self._index
        if self._verbose: log_start(self, '_gather_216')
        elem = self._tmp_276()
        if elem is not None:
            seq = self._loop0_217()
            if seq is not None:
    ...
    def _loop0_219(self): # type Optional[Any]
        # _loop0_219: block
        mark = self._index
        if self._verbose: log_start(self, '_loop0_219')
        children = []
        while True:
            block = self.block()
            if not block:
    ...
    def _loop1_220(self): # type Optional[Any]
        # _loop1_220: except_block
        mark = self._index
        if self._verbose: log_start(self, '_loop1_220')
        children = []
        while True:
            except_block = self.except_block()
            if not except_block:
    ...
    def _loop0_222(self): # type Optional[Any]
        # _loop0_222: block
        mark = self._index
        if self._verbose: log_start(self, '_loop0_222')
        children = []
        while True:
            block = self.block()
            if not block:
    ...
    def _loop1_223(self): # type Optional[Any]
        # _loop1_223: except_star_block
        mark = self._index
        if self._verbose: log_start(self, '_loop1_223')
        children = []
        while True:
            except_star_block = self.except_star_block()
            if not except_star_block:
    ...
    def _loop0_235(self): # type Optional[Any]
        # _loop0_235: ',' double_starred_kvpair
        mark = self._index
        if self._verbose: log_start(self, '_loop0_235')
        children = []
        while True:
            literal = self.expect_type(15)
            if not literal:
    ...
    def _gather_234(self): # type Optional[Any]
        # _gather_234: double_starred_kvpair _loop0_235
        mark = self._index
        if self._verbose: log_start(self, '_gather_234')
        elem = self.double_starred_kvpair()
        if elem is not None:
            seq = self._loop0_235()
            if seq is not None:
    ...
    def _loop0_248(self): # type Optional[Any]
        # _loop0_248: fstring_format_spec
        mark = self._index
        if self._verbose: log_start(self, '_loop0_248')
        children = []
        while True:
            fstring_format_spec = self.fstring_format_spec()
            if not fstring_format_spec:
    ...
    def _tmp_266(self): # type Optional[Any]
        # _tmp_266: starred_expression | (assignment_expression | expression !':=') !'='
        mark = self._index
    ...
    def _tmp_273(self): # type Optional[Any]
        # _tmp_273: expression ['as' star_target]
        mark = self._index
    ...
    def _tmp_274(self): # type Optional[Any]
        # _tmp_274: expressions ['as' star_target]
        mark = self._index
    ...
    def _tmp_275(self): # type Optional[Any]
        # _tmp_275: expression ['as' star_target]
        mark = self._index
    ...
    def _tmp_276(self): # type Optional[Any]
        # _tmp_276: expressions ['as' star_target]
        mark = self._index
    ...
    def _tmp_278(self): # type Optional[Any]
        # _tmp_278: assignment_expression | expression !':='
        mark = self._index
        if self._verbose: log_start(self, '_tmp_278')
        assignment_expression = self.assignment_expression()
        if assignment_expression:
            return assignment_expression
        self._index = mark
    ...
    def _tmp_279(self): # type Optional[Any]
        # _tmp_279: assignment_expression | expression !':='
        mark = self._index
        if self._verbose: log_start(self, '_tmp_279')
        assignment_expression = self.assignment_expression()
        if assignment_expression:
            return assignment_expression
        self._index = mark
    ...
    def _tmp_280(self): # type Optional[Any]
        # _tmp_280: 'as' star_target
        mark = self._index
        if self._verbose: log_start(self, '_tmp_280')
        literal = self.expect_type(521)
        if literal:
            star_target = self.star_target()
            if star_target:
    ...
    def _tmp_281(self): # type Optional[Any]
        # _tmp_281: 'as' star_target
        mark = self._index
        if self._verbose: log_start(self, '_tmp_281')
        literal = self.expect_type(521)
        if literal:
            star_target = self.star_target()
            if star_target:
    ...
    def _tmp_282(self): # type Optional[Any]
        # _tmp_282: 'as' star_target
        mark = self._index
        if self._verbose: log_start(self, '_tmp_282')
        literal = self.expect_type(521)
        if literal:
            star_target = self.star_target()
            if star_target:
    ...
    def _tmp_283(self): # type Optional[Any]
        # _tmp_283: 'as' star_target
        mark = self._index
        if self._verbose: log_start(self, '_tmp_283')
        literal = self.expect_type(521)
        if literal:
            star_target = self.star_target()
            if star_target:
