...
from test.support import (
    ...
    MISSING_C_DOCSTRINGS, impl_detail,
)
try:
    from test.test_import import no_rerun
except ImportError:
    # PyPy: test.test_import imports _testinternalcapi at module level, which
    # PyPy does not have.  no_rerun only suppresses refleak reruns, which PyPy
    # does not do either, so a no-op stand-in is exact here.
    def no_rerun(reason):
